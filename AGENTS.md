...
4. 先确认功能是否已实现。撤销/重做、文本美化、代码美化、GIF 和自动化测试目前未完成，不要在文档或 UI 中误称可用。
...
注意：当前 `pnpm-lock.yaml` 有重复映射键，frozen install 会失败。除非任务明确要求修复依赖，否则不要顺手重写或提交锁文件。临时诊断可使用 `pnpm install --lockfile=false`，但验证结果必须注明不是可复现安装。
...
代码职责、功能、公共 API、开发命令、依赖安装方式或已知限制发生变化时，更新 `DOCS/`。保持文档链接为相对仓库路径，并以当前代码为准。
